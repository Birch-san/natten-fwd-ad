...
class NattenConcatSplitRegistersBlock(Module):
  ...
  def forward(self, x: FloatTensor, registers: FloatTensor = None) -> FloatTensor:
    ...
    # alternatively:
    # qk_reg = (q.flatten(start_dim=-3, end_dim=-2) @ reg_k.mT).unflatten(-2, (x.size(-3), x.size(-2)))
    qk_reg = q @ reg_k.mT.unsqueeze(-3)
    qk_cat = cat([qk, qk_reg], dim=-1)
    a = qk_cat.softmax(dim=-1)
